...
'''usage eg:
python3 training_deep_learning_5-state-complete-sweeps-only.py ./ neut.fvec,hard.fvec,linkedHard.fvec,soft.fvec,linkedSoft.fvec,partialHard.fvec,linkedPartialHard.fvec,partialSoft.fvec,linkedPartialSoft.fvec 11 89 0.1 training_weights.hdf5 training.json training.npy
'''

if len(sys.argv)!=9:
  sys.exit("usage:\npython3 training_deep_learning_5-state-complete-sweeps-only.py fvecDir fvecFiles numSubWins numSumStatsPerSubWin validationSize weightsFileName jsonFileName npyFileName\n")
else:
...
sumstats=np.concatenate((sweeps[0],sweeps[1],sweeps[2],sweeps[3],sweeps[4]))
sumstats=sumstats.reshape(sumstats.shape[0],numSumStatsPerSubWin,numSubWins,1)
models=np.concatenate((np.repeat(0,len(sweeps[0])),np.repeat(1,len(sweeps[0])),np.repeat(2,len(sweeps[0])),np.repeat(3,len(sweeps[0])),np.repeat(4,len(sweeps[0]))))
sumstats,sumstats_val,models,models_val = train_test_split(sumstats,models,test_size=validationSize,random_state=42)
models=np_utils.to_categorical(models,5)
models_val=np_utils.to_categorical(models_val,5)
...
netlayers.add(Dropout(0.5))
netlayers.add(Dense(5, activation='softmax'))
