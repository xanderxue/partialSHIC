...
'''usage eg:
python3 training_deep_learning_binary-selection-presence.py ./ neut.fvec,hard.fvec,linkedHard.fvec,soft.fvec,linkedSoft.fvec,partialHard.fvec,linkedPartialHard.fvec,partialSoft.fvec,linkedPartialSoft.fvec 11 89 0.1 training_weights.hdf5 training.json training.npy
'''

if len(sys.argv)!=9:
  sys.exit("usage:\npython3 training_deep_learning_binary-selection-presence.py fvecDir fvecFiles numSubWins numSumStatsPerSubWin validationSize weightsFileName jsonFileName npyFileName\n")
else:
...
if fvecFiles.lower() in ["none","false","default"]:
  fvecFiles=['neut.fvec','linkedHard.fvec','linkedSoft.fvec','linkedPartialHard.fvec','linkedPartialSoft.fvec','hard.fvec','soft.fvec','partialHard.fvec','partialSoft.fvec']
else:
...
models=np.concatenate((np.repeat(0,len(sweeps[0])),np.repeat(0,len(sweeps[0])),np.repeat(0,len(sweeps[0])),np.repeat(0,len(sweeps[0])),np.repeat(0,len(sweeps[0])),np.repeat(1,len(sweeps[0])),np.repeat(1,len(sweeps[0])),np.repeat(1,len(sweeps[0])),np.repeat(1,len(sweeps[0]))))
sumstats,sumstats_val,models,models_val = train_test_split(sumstats,models,test_size=validationSize,random_state=42)
models=np_utils.to_categorical(models,2)
models_val=np_utils.to_categorical(models_val,2)
...
netlayers.add(Dropout(0.5))
netlayers.add(Dense(2, activation='softmax'))
