...
for pop in AOM BFM BFS CMS GAS GNS GWA KES UGS
  ...
  for ((i=0; $i < ${#chrArms[@]} ; i++));
    ...
    while [ $segmentStart -lt ${chrLens[$i]} ]
      do
      python2 empirical_convert_to_FVs.py empiricalData/ag1000g.phase1.ar3.1.haplotypes.$chrArm.h5 $chrArm ${chrLens[$i]} $segmentStart $segmentEnd $subWinSize $numSubWins 0.25 `python2 mosquito_data_files/stairwayPlotToPMisPol.py mosquito_data_files/$pop.meru_mela.sfs.sp.summary` mosquito_data_files/${pop}_partial_stats.txt mosquito_data_files/Anopheles-gambiae-PEST_CHROMOSOMES_AgamP3.accessible.fa mosquito_data_files/anc.meru_mela.$chrArm.fa mosquito_data_files/samples_pops.txt $pop empiricalData/sumstats/$pop/$chrArm.$segmentStart.stats empiricalData/FVs/$pop/$chrArm.$segmentStart.fvec > empiricalData/logs/$pop/$chrArm.$segmentStart.log
